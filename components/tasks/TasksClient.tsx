...
const columns: { id: TaskColumn; label: string; dotColor: string }[] = [
  ...
  { id: 'OWNER_CHECKIN', label: 'OWNER CHECK-IN', dotColor: 'bg-[#17C4BE]' },
]
        ...
        <div className="flex bg-white rounded-full p-1 shadow-sm border border-slate-200">
          <button
            ...
            className={`px-6 py-2 text-sm font-bold rounded-full transition-colors ${
              view === 'board' ? 'bg-slate-50 text-[#17C4BE] shadow-sm' : 'text-slate-500'
            }`}
          >
            Board View
          </button>
          <button
            ...
            className={`px-6 py-2 text-sm font-medium rounded-full transition-colors ${
              view === 'list' ? 'bg-slate-50 text-[#17C4BE] shadow-sm' : 'text-slate-500'
            }`}
          >
            List View
          </button>
    ...
    >
      ...
      {colId === 'OWNER_CHECKIN' && (
        <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
          <Phone className="w-3.5 h-3.5 text-[#17C4BE]" />
          Contact Ready
        </div>
      )}
    </div>
