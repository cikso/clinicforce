  ...
  return (
    <PageShell title="Team & Users">
      <div className="max-w-2xl space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#00D68F] hover:bg-[#0e4f79] text-white text-xs font-bold rounded-xl transition-colors"
            >
            ...
            {team.map(member => {
              ...
              return (
                <div key={member.id} className="flex items-center gap-4 p-3 rounded-xl border border-slate-100 bg-slate-50/50">
                  <div className="w-9 h-9 rounded-full bg-[#00D68F] flex items-center justify-center shrink-0 text-white text-xs font-bold">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            ...
            <form onSubmit={handleAddUser} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="Jane Smith"
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00D68F] focus:border-transparent transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wide">Role *</label>
                  <select value={role} onChange={e => setRole(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#00D68F] focus:border-transparent transition-all">
              ...
              <button type="submit" disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#00D68F] hover:bg-[#0e4f79] text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-60">
        ...
        {/* How it works */}
        <div className="bg-[#E6FBF2] border border-[#c8e0f4] rounded-2xl p-5">
          <p className="text-xs font-bold text-[#00D68F] uppercase tracking-wide mb-2">How staff accounts work</p>
