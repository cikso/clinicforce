    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="flex bg-slate-100 rounded-full p-1 shadow-inner border border-slate-200">
          <button
            onClick={() => setTab('live')}
            className={`px-6 py-2 text-sm font-bold rounded-full transition-colors ${tab === 'live' ? 'text-[#0f5b8a] bg-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Live Queue ({liveCalls.length})
          </button>
          <button
            onClick={() => setTab('archived')}
            className={`px-6 py-2 text-sm font-medium rounded-full transition-colors ${tab === 'archived' ? 'text-[#0f5b8a] bg-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Archived
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-6">
          {/* Live Session Widget */}
          <div className="bg-[#0f5b8a] rounded-[2rem] p-6 text-white shadow-md relative overflow-hidden">
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-200">
            <div className="flex items-center gap-2 mb-6">
              <Sparkles className="w-5 h-5 text-[#0f5b8a]" />
          ...
          <div className="bg-slate-50 rounded-[2rem] p-6 border border-slate-200 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-[#0f5b8a]">Transcription Online</span>
            </div>
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1 h-1 bg-slate-200 rounded-full overflow-hidden">
            <div className={`h-full rounded-full ${isCritical ? 'bg-[#0f5b8a] w-1/3' : isUrgent ? 'bg-slate-300 w-0' : 'bg-slate-300 w-1/2'}`}></div>
          </div>
          <button className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
            isCritical ? 'bg-[#0f5b8a] text-white hover:bg-[#0c4a70] shadow-sm' : 'border-2 border-slate-200 text-slate-400 hover:border-slate-300'
          }`}>
