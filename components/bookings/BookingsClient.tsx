    ...
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
          ...
          <h3 className="text-4xl font-bold text-[#17C4BE] mb-3">{confirmed.length + checkedIn.length} Booked</h3>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
          ...
          <div className="w-full bg-slate-200 rounded-full h-2.5">
            <div className="bg-[#17C4BE] h-2.5 rounded-full w-[84%]"></div>
          </div>
        </div>
        <div className="bg-[#eaf8f4] rounded-3xl p-6 border border-[#ccede4]">
          ...
          <h3 className="text-4xl font-bold text-[#17C4BE] mb-3">2:30 PM</h3>
          <p className="text-sm font-semibold text-teal-700 flex items-center gap-1.5">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex bg-slate-100 p-1.5 rounded-full">
          <button onClick={() => setView('list')} className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-semibold transition-colors ${view === 'list' ? 'bg-white shadow-sm text-[#17C4BE]' : 'text-slate-500'}`}>
            <List className="w-4 h-4" />
            List View
          </button>
          <button onClick={() => setView('calendar')} className={`flex items-center gap-2 px-6 py-2 rounded-full text-sm font-semibold transition-colors ${view === 'calendar' ? 'bg-white shadow-sm text-[#17C4BE]' : 'text-slate-500'}`}>
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="px-2.5 py-0.5 bg-[#17C4BE] text-white text-[10px] font-bold rounded-full uppercase tracking-wider">
              ...
              return (
                <div key={booking.id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="flex items-center gap-2 px-5 py-2.5 bg-[#17C4BE] text-white rounded-full text-sm font-bold hover:bg-[#13ADA8] transition-colors">
                      <CheckCircle2 className="w-4 h-4" />
                      Approve & Notify
                    </button>
                  </div>
                ...
                <div className="bg-slate-50 rounded-2xl p-4">
                  ...
                  <div className="flex gap-1 mb-2">
                    {[true, true, false, false].map((on, i) => (
                      <div key={i} className={`h-2 flex-1 rounded-full ${on ? 'bg-[#17C4BE]' : 'bg-slate-200'}`}></div>
                    ))}
                  </div>
