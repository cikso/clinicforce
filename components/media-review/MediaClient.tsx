        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex bg-slate-100 p-1.5 rounded-full w-fit">
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-colors ${tab === 'review' ? 'bg-white shadow-sm text-[#0f5b8a]' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Review Needed ({reviewCount})
            </button>
            <button
              onClick={() => setTab('flagged')}
              className={`px-6 py-2 rounded-full text-sm font-bold transition-colors ${tab === 'flagged' ? 'bg-white shadow-sm text-[#0f5b8a]' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Flagged ({flaggedCount})
            </button>
            <button
              onClick={() => setTab('all')}
              className={`px-6 py-2 rounded-full text-sm font-bold transition-colors ${tab === 'all' ? 'bg-white shadow-sm text-[#0f5b8a]' : 'text-slate-600 hover:text-slate-900'}`}
            >
              All Media
            </button>
          </div>
      ...
      {selected && (
        <div className="w-[420px] bg-white border-l border-slate-200 flex flex-col overflow-y-auto shrink-0 shadow-2xl z-20">
          <div className="p-6 border-b border-slate-100 flex items-start justify-between bg-white sticky top-0 z-10">
            <div>
              <h2 className="text-xl font-bold text-[#0f5b8a] mb-1">
          ...
          <div className="p-6 space-y-6 flex-1">
            ...
            {selected.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {selected.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 bg-[#f0f6ff] text-[#0f5b8a] text-xs font-bold rounded-full"
                  >
                    {tag}
                  </span>
                ))}
            ...
            {selected.aiInsights && selected.aiInsights.length > 0 && (
              <div className="bg-slate-50/50 rounded-3xl p-5 border border-slate-100">
                <div className="flex items-center gap-2 mb-4">
                  <Sparkles className="w-4 h-4 text-[#0f5b8a]" />
                  <h3 className="text-[10px] font-bold text-[#0f5b8a] uppercase tracking-widest">
                    AI Insights
                  </h3>
                </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <button className="w-full py-4 bg-[#0f5b8a] text-white rounded-full text-sm font-bold hover:bg-[#0c4a70] transition-colors shadow-sm">
  ...
  return (
    <div
      ...
      className={`relative rounded-[2rem] overflow-hidden cursor-pointer group shadow-sm border-2 transition-all duration-200 ${
        selected ? 'border-[#0f5b8a]' : 'border-transparent hover:border-slate-300'
      }`}
      style={{ height: '280px' }}
    >
      {item.thumbnailUrl ? (
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#0f5b8a]/80 via-[#0f5b8a]/10 to-transparent"></div>
      ...
      <div className="absolute bottom-5 left-5 right-5">
        ...
        <div className="flex flex-wrap gap-1.5">
          {item.tags.slice(0, 2).map((tag) => (
            <span
              key={tag}
              className="px-2.5 py-1 bg-[#0c4a70]/80 text-teal-50 text-xs font-bold rounded-full"
            >
              {tag}
            </span>
          ))}
