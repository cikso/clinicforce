        ...
        {actions.map(({ icon: Icon, label }) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-teal-50 text-[#00D68F] rounded-md group-hover:bg-teal-100 transition-colors">
                <Icon className="w-4 h-4" />
              </div>
              <span className="text-sm font-medium text-slate-700 group-hover:text-slate-900">{label}</span>
            </div>
            <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-[#00D68F] transition-colors" />
          </button>
        ))}
      </div>
    </section>
