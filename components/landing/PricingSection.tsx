  ...
  return (
    <div
      className={`relative flex flex-col rounded-3xl border p-8 ${
        highlighted
          ? 'border-[#00D68F] bg-white shadow-[0_20px_50px_rgba(0,214,143,0.15)]'
          : 'border-[#E5EAF0] bg-white shadow-[0_10px_30px_rgba(15,23,42,0.04)]'
      }`}
    >
      {highlighted && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <span className="rounded-full bg-[#00D68F] px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-white">
            Most Popular
          </span>
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-[#566275]">{description}</p>
      </div>

      <div className="mb-6">
        <span className="text-4xl font-bold tracking-[-0.05em] text-[#1A1A1A]">{price}</span>
        {period && <span className="text-base text-[#566275]">/{period}</span>}
        {daily && <p className="mt-1 text-sm text-[#566275]">{daily}</p>}
      </div>

      <ul className="mb-8 flex-1 space-y-3">
        {features.map((f) => (
          <li key={f} className="flex items-start gap-2.5">
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#00D68F]" />
            <span className="text-sm leading-relaxed text-[#566275]">{f}</span>
          </li>
        ))}
      </ul>
      ...
      <button
        ...
        className={`w-full rounded-2xl px-6 py-4 text-sm font-semibold transition ${
          highlighted
            ? 'bg-[#00D68F] text-white shadow-[0_12px_24px_rgba(0,214,143,0.22)] hover:bg-[#00B578]'
            : name === 'Enterprise'
              ? 'border border-[#E5EAF0] bg-white text-[#1A1A1A] hover:bg-[#f9fbfc]'
              : 'bg-[#00D68F] text-white hover:bg-[#00B578]'
        }`}
      >
        {cta}
      </button>
