...
// ── Static fallback (shown while loading or if Supabase returns no rows) ──────
  ...
  return (
    <aside className="flex flex-col w-56 shrink-0 h-screen overflow-y-auto" style={{ background: '#0f1729', borderRight: '1px solid #1e2d4a' }}>
      ...
      <nav className="flex-1 px-2 pt-5 pb-2">
        <p className="px-3 mb-2 text-[9px] font-bold uppercase tracking-widest select-none" style={{ color: '#3d5070' }}>Workspace</p>
        <div className="space-y-0.5">
          {NAV_ITEMS.map(({ label, href, icon: Icon, active }) => (
            ...
            >
              <Icon className="w-4 h-4 shrink-0" style={{ color: active ? '#4fd1c5' : '#3d5070' }} />
              {label}
            </Link>
          ))}
        </div>
        <p className="px-3 mt-6 mb-2 text-[9px] font-bold uppercase tracking-widest select-none" style={{ color: '#3d5070' }}>Admin</p>
        <div className="space-y-0.5">
      ...
      <div className="px-3 pb-4" style={{ borderTop: '1px solid #1e2d4a', paddingTop: '1rem' }}>
        <div className="rounded-xl p-3" style={{ background: '#1a2844' }}>
          <div className="flex items-center gap-2.5 mb-2.5">
            <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-lg" style={{ background: '#0f3460' }}>🐕</div>
            <div className="min-w-0">
          ...
          <button className="w-full text-[11px] font-semibold py-1.5 rounded-lg transition-opacity hover:opacity-80"
...
export default function InsightsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/insights')
      .then(r => r.json())
      .then((live: InsightsData) => {
        // Only replace if we got real data back (not an error object)
        if (live?.kpis) setData(live)
      })
      .catch(() => { /* keep fallback */ })
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200">
                ...
                <input type="text" placeholder="Search…" className="text-[12px] bg-transparent outline-none text-slate-600 w-28 placeholder-slate-400" />
              </div>
              ...
              <div className="flex items-center gap-2.5 pl-3 border-l border-slate-200">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-white" style={{ background: '#0d9488' }}>SJ</div>
                <div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-6 gap-3">
            <KpiCard
              label="Total Calls"       value={String(kpis.totalCalls)}    sub="last 7 days"
              icon={Phone}         iconBg="#f0fdf4" iconColor="#16a34a"
              trendMeta={trends.totalCalls}   loading={loading}
            />
            <KpiCard
              label="Appointments"      value={String(kpis.appointments)}   sub="booking intent"
              icon={Calendar}      iconBg="#eff6ff" iconColor="#3b82f6"
              trendMeta={trends.appointments} loading={loading}
            />
            <KpiCard
              label="Urgent Cases"      value={String(kpis.urgentCases)}    sub="flagged"
              icon={AlertTriangle} iconBg="#fef2f2" iconColor="#ef4444"
              trendMeta={trends.urgentCases}  loading={loading}
            />
            <KpiCard
              label="Avg Handle Time"   value={kpis.avgHandleTime > 0 ? `${kpis.avgHandleTime}m` : '—'} sub="per call"
              icon={Clock}         iconBg="#faf5ff" iconColor="#9333ea"
              loading={loading}
            />
            <KpiCard
              label="AI Containment"    value={kpis.aiContainment > 0 ? `${kpis.aiContainment}%` : '—'} sub="fully resolved"
              icon={Zap}           iconBg="#f0fdf4" iconColor="#0d9488"
              trendMeta={trends.aiContainment} loading={loading}
            />
            <KpiCard
              label="Booking Conv."     value={kpis.bookingConv > 0 ? `${kpis.bookingConv}%` : '—'} sub="call to booking"
              icon={Target}        iconBg="#fff7ed" iconColor="#ea580c"
              trendMeta={trends.bookingConv}  loading={loading}
            />
          </div>
                    ...
                    <div key={label}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-[12px] font-bold tabular-nums"
                          style={{ color: pct >= 80 ? '#0d9488' : pct >= 60 ? '#d97706' : '#ef4444' }}>
                          {pct}%
                        </span>
                      </div>
                      <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                        <div className="h-full rounded-full transition-all duration-700" style={{
                          width: `${pct}%`,
                          background: pct >= 80 ? '#0d9488' : pct >= 60 ? '#d97706' : '#ef4444',
                        }} />
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-[0_1px_4px_rgba(15,39,68,0.06)]">
              <div className="mb-4">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {loading ? 'Loading…' : kpis.totalCalls > 0
                    ? 'Call volume by day and hour — darker = busier'
                    : 'No calls recorded yet — heatmap will populate automatically'
                  }
                </p>
              </div>
